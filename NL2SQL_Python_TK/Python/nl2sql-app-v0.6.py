...
from settings_encryption import SettingsEncryption  # Import the encryption utility
import sqlalchemy
from sqlalchemy import create_engine, text, inspect
from sqlalchemy.exc import SQLAlchemyError
from urllib.parse import quote_plus  # Add this import for URL escaping
...
class NL2SQLApp:
    def __init__(self, root):
        ...
        # Initialize settings encryption utility with path to key file
    ...
    def load_config(self):
        ...
        config_file = os.path.join(self.tmp_dir, "nl2sql_config.enc")
        ...
        if os.path.exists(config_file):
            try:
                ...
                if "database" in config:
                    ...
                    # Create SQLAlchemy engine after loading config
                    self._create_engine()
    ...
    def _create_engine(self):
        """Create SQLAlchemy engine from current configuration"""
        try:
    ...
    def save_config(self):
        ...
        try:
            ...
            messagebox.showinfo("Configuration", "Configuration saved securely.")
        except Exception as e:
    ...
    def show_settings(self):
        ...
        # Create a variable to hold the API key, either the actual key or masked version
        api_key_var = tk.StringVar()
        if self.openai_api_key:
            # If a key exists, set a placeholder value for display
            api_key_var.set(self.openai_api_key)
        
        # Create API key entry with asterisk masking
    ...
    def test_db_connection(self, host, user, password, database, port):
        ...
        try:
            ...
            # Create a test connection string for SQLAlchemy
    ...
    def save_settings(self, host, user, password, database, port, api_key, model, window):
        ...
        try:
            ...
            # Create SQLAlchemy engine with new settings
            self._create_engine()
    ...
    def execute_query(self):
        ...
        try:
            ...
            log_file = os.path.join("logging", f"error_{today}.log")
    ...
    def get_db_schema(self):
        """Get the database schema using SQLAlchemy"""
        try:
            if not self.engine:
    ...
    def execute_sql(self, sql_query):
        ...
        try:
            ...
            # Execute query and convert to pandas DataFrame using SQLAlchemy
            df = pd.read_sql_query(sql=text(sql_query), con=self.engine)
            return df
